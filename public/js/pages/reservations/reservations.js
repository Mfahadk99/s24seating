document.addEventListener("DOMContentLoaded", function () {
  ...
  if (reservationsDataElement) {
    reservationsData = JSON.parse(
      reservationsDataElement.getAttribute("data-reservations")
    );
  }
...
function preventTableDragging() {
  ...
  document.addEventListener("dragstart", function (e) {
    if (
      e.target.classList.contains("ap-dropped-table") ||
  ...
  document
    .querySelectorAll(".ap-dropped-table, .canvas-table")
    .forEach((table) => {
...
function initializeFullscreen() {
  ...
  const isFullscreen =
    localStorage.getItem("reservationsIsFullscreen") === "true";
  if (isFullscreen) {
...
function selectTable(tableId) {
  ...
  document
    .querySelectorAll(".canvas-table.selected, .reservation-row.selected")
    .forEach((el) => {
      el.classList.remove("selected");
    });
  // Add selection to canvas table
  const canvasTable = document.querySelector(
    `.canvas-table[data-table-id="${tableId}"]`
  );
  if (canvasTable) {
  ...
  const listItems = document.querySelectorAll(
    `.reservation-row[data-table-id="${tableId}"]`
  );
  ...
  document.querySelectorAll(".section-toggle-btn").forEach((button) => {
    button.addEventListener("click", function () {
      ...
      if (
        ...
        !sectionContent.classList.contains("expanded")
      ) {
        // Expand section
