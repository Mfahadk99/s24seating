$("#chatbotSelect")
  .change(function () {
    ...
    if (selectedChatbotId) {
      LoadFlowDiagram(selectedChatbotId);
      $("#mainContainer").show();
    }
  })
...
// TODO:REMOVE:
// $("#chatbotSelect").val("66f92044f9dd8878481c2dfe").change();
// LoadFlowDiagram('66f92044f9dd8878481c2dfe')
// $('#mainContainer').show();
...
// Add Event Node
function addEventNode(pos_x, pos_y) {
  const nodeId = Date.now(); // Unique ID
  const template = `
  <div class="node event">
    <div class="title-box" >
      <input type="text" id="eventName-${nodeId}" class="event-name" df-event-name placeholder="Enter event name">
    ...
    <div class="box collapse custom-collapse" id="eventCollapse-${nodeId}">
      ...
      <textarea id="eventDescription-${nodeId}" class="form-control" df-event-description placeholder="Enter event description"></textarea>
    </div>
  </div>
  `;
    ...
    send_sms: `
      <div class="node action">
          ...
          <div class="box collapse custom-collapse" id="smsCollapse-${nodeId}">
              ...
              <input type="tel"  df-action-input1 df-event-name id="sms-number" placeholder="Enter recipient's phone number...">
...
async function saveFlowDiagram() {
  try {
    ...
    const response = await fetch(
      `/flow-controller/flow-diagram?chatbotId=${chatbotId}`,
      {
        method: "POST",
        headers: {
...
async function LoadFlowDiagram(chatbotId) {
  try {
    editor.clear();
    $("#alertDiv").fadeIn(500);
